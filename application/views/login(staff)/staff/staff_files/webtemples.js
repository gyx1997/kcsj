...
$(function () {
    ...
    //        $(this).css1("background-position", "0px -120px");
    //    }).bind('mouseleave', function () {
    //        $(this).css1("background-position", "0px -90px");
    //    });
    ...
    $('.sub_edit').click(function (event) {
        ...
        //        if ($(this).parent().parent().children('.us_line').css1('display') == 'block')//如果已经有菜单，则点击按钮回收菜单
        //        {
        ...
        //            $(this).css1("background-position", "0px -120px").unbind('mouseenter').unbind('mouseleave');
    ...
    $(".us_page a:eq(0)").hover(
     function () {
         ...
         $(this).append("<img src='/UserHome/images1/left_w.gif'/>");
     },
   function () {
       $(this).empty();
       $(this).append("<img src='/UserHome/images1/left_b.gif'/>");
   }
    );
    ...
    $(".us_page a:eq(1)").hover(
     function () {
         ...
         $(this).append("<img src='/UserHome/images1/right_w.gif'/>");
     },
   function () {
       $(this).empty();
       $(this).append("<img src='/UserHome/images1/right_b.gif'/>");
   }
    );
    ...
    $("#Name_Next").click(function () {
        ...
        if (ismpt == "") {
            datacheck.empty();
            datacheck.append("<img src='../../images1/cross.png' />" + msg.notEmpty + "");
        }
        else {
            ...
            //  alert((msg));
            datacheck.empty();
            datacheck.append("<img src='../../images1/tick.png' />" + msg.Congratulations + "");
            close_div();
    ...
    $("#chril_Domain").keyup(function () {//检查数据库是否已经存在此二级域名
        ...
        if ((isornot == false) || (s_nme.length < 5)) {
            ...
            chril_Domain_check.append("<img src='../../../images1/cross.png' id='imgsrc' />" + msg.zhuhe + ""); //5-16位字母或数字组合！
        }
        if (s_nme.length >= 5 && isornot == true) {
            chril_Domain_check.empty();
            chril_Domain_check.append("<img src='/images1/xiaozhanggei.gif' />");
            $.post("/Ajax_WebServer/Ajax_Check_Data.ashx", { s_nme: s_nme }, function (data) {
                if (data == 0) {
                    chril_Domain_check.empty();
                    chril_Domain_check.append("<img src='../../../images1/cross.png' id='imgsrc'/>" + msg.aldrdome + ""); //此二级域名已经存在请换一个！
                }
                else if (data == 1) {
                    chril_Domain_check.empty();
                    chril_Domain_check.append("<img src='../../../images1/tick.png' />" + msg.domereg + ""); //此二级域名可以注册！
    ...
    $("#check_domain").click(function () {
        ...
        if (New_Domain.length > 3) {
            New_Domain_Msg.empty();
            New_Domain_Msg.append("<img src='/images1/xiaozhanggei.gif' />");
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    if (data == 0)
                    { New_Domain_Msg.empty(); New_Domain_Msg.append("<img src='../../../images1/cross.png' id='imgsrc1' />" + msg.alerdreg + ""); } //该域名已被注册！
                    else if (data == 1)
                    { New_Domain_Msg.empty(); New_Domain_Msg.append("<img src='../../../images1/tick.png' id='imgsrc9' />" + msg.okreg + ""); } //該域名可以註冊！
                    else if (data == 2) {
                        New_Domain_Msg.empty(); New_Domain_Msg.append("<img src='../../../images1/cross.png' id='imgsrc1' />" + msg.errsate + ""); //该域名异常！
                    } else if (data == "TimeOut") {
    ...
    $("#Has_Domain").keyup(function () {
        ...
        if ($(this).val().length < 5) {
            ...
            Has_Domain_Msg.append("<img src='../../../images1/cross.png' />" + msg.limdome + ""); //域名不低於4位有效字符！
        }
        else {
            if (isdomain($(this).val()) == false) {
                Has_Domain_Msg.empty();
                Has_Domain_Msg.append("<img src='../../../images1/cross.png' />" + msg.plseas + ""); //請輸入正確的域名！
            }
            else {
                Has_Domain_Msg.empty();
                Has_Domain_Msg.append("<img src='../../../images1/tick.png' />" + msg.pass + ""); //檢測通過！
            }
        }
